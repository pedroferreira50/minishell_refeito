...
/*   Updated: 2025/04/24 17:56:35 by scarlos-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_command(char *command, char **args, t_shell *shell, \
			pid_t *pids, t_command_data *data)
{
	char	*full_path;
...
static pid_t	*init_execution(t_command_data *data, t_exec_state *state,
					t_shell *shell)
{
	pid_t	*pids;
...
static void	fork_child(t_command_data *data, t_exec_state *state,
				t_shell *shell, pid_t *pids)
{
	...
	if (!has_builtin)
		execute_command(data->commands[state->i], data->arguments[state->i],
			shell, pids, data);
	free_command_data(data);
	exit(1);
}

static void	manage_parent(pid_t pid, pid_t *pids, t_exec_state *state,
			t_command_data *data)
{
	if (pid > 0)
	{
...
static void	run_pipeline(t_command_data *data, t_exec_state *state,
			t_shell *shell, pid_t *pids)
{
	pid_t	pid;
